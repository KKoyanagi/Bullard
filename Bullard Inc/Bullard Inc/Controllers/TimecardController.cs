...
namespace Timecard.Controllers
{
    public class TimecardController : Controller
    {
        ...
        public async Task<ActionResult> EmpJobSubmit(int day_id, Job job)  
        {
            ...
            EmployeeDay emp;
            string resp;
            string getTimesheet = url + "employeedays/" + job.EmployeeDay_Id;
            HttpResponseMessage responseMessage = await client.GetAsync(getTimesheet);
            if (responseMessage.IsSuccessStatusCode)
            {
                resp= responseMessage.Content.ReadAsStringAsync().Result;
                emp = JsonConvert.DeserializeObject<EmployeeDay>(resp);
                string empJobAddURL = url + "jobs";
                responseMessage = await client.PostAsJsonAsync(empJobAddURL, job);
                System.Net.HttpStatusCode response = responseMessage.StatusCode;
                Debug.WriteLine(responseMessage.Content);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("/empjobview/" + emp.Timesheet_Id + "/"+ day_id);
                }
                return RedirectToAction("Error" + response);
            }
       ...
       //[Route("timecard/empjobview/{empdayid}/{day_id}/empjobedit")]
        ...
        public async Task<ActionResult> EmpJobUpdate(int day_id, [Bind(Include = "Job_Id,EmployeeDay_Id,Project_Id,ActivityCode,Hours,Mileage,Lunch,")] Job job)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                responseMessage = await client.PutAsJsonAsync(empJobUpdateURL, job);
                System.Net.HttpStatusCode response = responseMessage.StatusCode;
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("/empjobview/" + emp.Timesheet_Id + "/" + day_id);
        ...
        public async Task<ActionResult> EmpJobDelete(int empdayid, int day_id, int? id)
        {
            EmployeeDay emp;
            string resp;
            string getTimesheet = url + "employeedays/" + empdayid;
            HttpResponseMessage responseMessage = await client.GetAsync(getTimesheet);
            if (responseMessage.IsSuccessStatusCode)
            {
                resp = responseMessage.Content.ReadAsStringAsync().Result;
                ...
                responseMessage = await client.DeleteAsync(empJobDeleteURL);
                System.Net.HttpStatusCode response = responseMessage.StatusCode;
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("/empjobview/" + emp.Timesheet_Id + "/" + day_id);
